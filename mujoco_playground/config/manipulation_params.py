...
def brax_ppo_config(
  ...
  if env_name == "AlohaHandOver":
    ...
    rl_config.num_timesteps = 500_000 #10_000_000 #200_000_000
    rl_config.num_evals = 5 #10 #20
    rl_config.num_minibatches = 32
    rl_config.unroll_length = 10 #40
    rl_config.num_updates_per_batch = 2 #4
    rl_config.discounting = 0.97
    rl_config.learning_rate = 3e-4#3e-4
    rl_config.entropy_cost = 1e-2
    rl_config.num_envs = 128 #512 #8192
    rl_config.batch_size = 256
    rl_config.network_factory = config_dict.create(
        policy_hidden_layer_sizes=(512, 256, 128),
        value_hidden_layer_sizes=(512, 256, 128),
...
def brax_ars_config(
  ...
  if env_name == "DualUR5eBoxlift":
    ...
    rl_config.num_evals = 20
    rl_config.number_of_directions = 250
    rl_config.top_directions = 100
    rl_config.step_size = 0.03 #0.015
    rl_config.exploration_noise_std = 0.5 #0.025
    rl_config.num_eval_envs = 128
    rl_config.reward_shift = 0.0
    rl_config.seed = 0
  else:
